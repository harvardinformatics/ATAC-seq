...
![Adapter removal](adapter_removal.png)
...
2) Remove the mitochondrially-derived reads after alignment.  (The issue here is that it’s not as simple as a one-line awk.  The bitwise FLAG of the SAM needs to be corrected for “not properly paired” alignments that include chrM.  Maybe filtering with -f 0x2 would do the trick, but really shouldn’t require this.)
...
![Alignment types](alignments.png)
Figure 3. Paired-end alignment possibilities.  A: “Properly paired” alignments.  Reads that are properly paired align in opposite orientations on the same reference sequence (chromosome).  The reads may overlap to some extent (bottom).  B: “Singleton” alignments.  A read can be not properly paired for several reasons: if its mate is unaligned (upper left), aligns to a different chromosome (upper right), aligns in the same orientation (middle), or aligns in the correct orientation but at an invalid distance (bottom).


An important consideration is which types of alignments should be analyzed by MACS2, and how those alignments should be interpreted.  The analysis mode is set by the ‘-f’ argument.  Here are the options with MACS2:
